...
export class LPBond extends Bond {
    ...
    public getShrkAmount(networkID: Networks, provider: StaticJsonRpcProvider) {
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isShrk = token1.toLowerCase() === addresses.SHRK_ADDRESS.toLowerCase();

        return isToken ? this.toTokenDecimal(false, isShrk ? reserve0 : reserve1) : this.toTokenDecimal(true, isShrk ? reserve1 : reserve0);
    }

    private toTokenDecimal(isShrk: boolean, reserve: number) {
        return isShrk ? reserve / Math.pow(10, 9) : reserve / Math.pow(10, 18);
    }
}
